...
from .crypto import HomomorphicPrivateKey, HomomorphicPublicKey, PRFKey
from . import network
...
# AggFT Rounds Metadata
# Specific for using homomorphic encryption.
# Base class for the DC and SM versions.
# Don't use directly.
@dataclass(frozen=True)
class HomomorphicMetadata(Metadata):
    # Homomorphic Public Key
    pk: HomomorphicPublicKey


# AggFT Rounds Metadata
# Specific for a DC using homomorphic encryption.
@dataclass(frozen=True)
class DCHomomorphicMetadata(HomomorphicMetadata):
    # Homomorphic Secret Key
    sk: HomomorphicPrivateKey


# AggFT Rounds Metadata
# Specific for an SM using homomorphic encryption.
@dataclass(frozen=True)
class SMHomomorphicMetadata(HomomorphicMetadata):
...
def is_valid_homomorphic_metadata(m: HomomorphicMetadata) -> bool:
    # Should be a valid metadata
    return is_valid_metadata(m)


def is_valid_dc_homomorphic_metadata(m: DCHomomorphicMetadata) -> bool:
    # Should be a valid homomorphic metadata
    return is_valid_homomorphic_metadata(m)


def is_valid_sm_homomorphic_metadata(m: SMHomomorphicMetadata) -> bool:
    # Should be a valid homomorphic metadata
    return is_valid_homomorphic_metadata(m)
