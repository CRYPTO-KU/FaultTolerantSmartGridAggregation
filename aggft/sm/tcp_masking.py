...
def run_forever(port: network.Port, dc_address: Tuple[network.Address, bool], id: int, sm_addresses: Tuple[Tuple[network.Address, bool], ...], meta: RoundMetadata, k: int, prf_key: bytes, test: Dict):
    ...
    run_single_round(0, port, dc_address, id, sm_addresses, meta, k, prf_key, test)
    log.info(f"Round {0} ended.")

def run_single_round(round: int, port: network.Port, dc_address: Tuple[network.Address, bool], id: int, sm_addresses: Tuple[Tuple[network.Address, bool], ...], meta: RoundMetadata, k: int, prf_key: bytes, test: Dict):
    # Wait for the right time to start round
    round_start = meta.start + meta.round_len * round
    sleep(time.remaining_until(round_start + 0.1))
    test["start"] = now()
...
def run_phase_2(round: int, port: network.Port, dc_address: Tuple[network.Address, bool], id: int, sm_addresses: Tuple[Tuple[network.Address, bool], ...], meta: RoundMetadata, s: int, k: int):
    round_start = meta.start + meta.round_len * round
    phase_2_end = round_start + meta.round_len
    requests = network.listen_multiple(port, phase_2_end, "<START>", "<END>", phase_2_early_stopper(round, id, tuple(map(lambda address: address[0], sm_addresses))))
    validator = phase_2_request_validator(round, id, tuple(map(lambda address: address[0], sm_addresses)))
    ...
    while not is_last(l_rem, l_act, meta.n_min):
        ...
        if sm_addresses[next_sm][1]:
            is_ok = False
        else:
            is_ok = network.send(
                sm_addresses[next_sm][0],
                json.dumps(data),
                phase_2_end,
                "<START>", "<END>"
            )
